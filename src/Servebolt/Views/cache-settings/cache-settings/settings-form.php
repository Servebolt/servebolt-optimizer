    ...
    <table class="form-table">
        ...
        <tbody id="sb-html-cache-form" <?php echo ($htmlCacheActive ? '' : ' style="display: none;"'); ?>>
            <?php if ($acdIsActive): ?>
                <tr class="sb-config-field-general <?php if (!$cachePurgeIsActive) echo ' sb-config-field-hidden'; ?>">
                    <th scope="row">
                        ...
                        <span class="description sb-new-badge"><?php _e('New in 3.6.1', 'servebolt-wp'); ?></span>
                    </th>
                    <td>
                        <?php
                            ...
                            <?php foreach ($availableHeaders as $key => $headerName) : ?>
                                ...
                                <div style="margin-bottom: 10px;">
                                    ...
                                    <?php if (!empty($option['description'])) : ?>
                                        <p class="description sb-option-description"><?php echo esc_html($option['description']); ?></p>
                                    <?php endif; ?>
                                </div>
                            <?php endforeach; ?>
            ...
            <?php if ($acdIsActive && $woocommerceIsActive): ?>
                <tr class="sb-config-field-general <?php if (!$cachePurgeIsActive) echo ' sb-config-field-hidden'; ?>">
                    <th scope="row">
                        <?php _e('Limit Cache purging on WooCommerce product updates', 'servebolt-wp'); ?>
                        <span class="description sb-new-badge"><?php _e('New in 3.6.2', 'servebolt-wp'); ?></span>
                    </th>
                    <td>
                        <fieldset>
                            <legend class="screen-reader-text"><span><?php _e('Select which cache groups to avoid purging on product updates', 'servebolt-wp'); ?></span></legend>
                            <?php foreach ($woocommerceProductEventPurgeOptions as $optionKey => $optionConfig) : ?>
                                <div style="margin-bottom: 10px;">
                                    ...
                                    <p class="description sb-option-description"><?php echo esc_html($optionConfig['description']); ?></p>
                                </div>
                            <?php endforeach; ?>
                            <p class="description"><?php _e('When enabled for a cache group, Product updates will only purge that group on create, delete, and out-of-stock events. When disabled, all product updates will keep purging that group.', 'servebolt-wp'); ?></p>
                        </fieldset>
                    </td>
                </tr>
            <?php endif; ?>
            ...
            <tr>
                ...
                <td>
                    ...
                    <table class="wp-list-table widefat striped" id="html-cache-ids-to-exclude-table">
                        ...
                        <tbody id="the-list">
                            <tr class="no-items<?php if (count($idsToExclude) > 0) echo ' hidden'; ?>">
                                <td colspan="100%"><?php _e('No posts set to be excluded', 'servebolt-wp'); ?></td>
                            </tr>
