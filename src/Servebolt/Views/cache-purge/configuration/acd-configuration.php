...
<tbody class="sb-config-field-general sb-config-field-acd <?php if ( ! $settings['cache_purge_switch'] || $settings['cache_purge_driver'] !== 'acd' ) echo 'sb-config-field-hidden'; ?>">
    <tr>
        <th scope="row" colspan="100%" style="padding-bottom: 5px;">
            ...
            <p style="font-weight: normal;"><?php _e('The information below is read from the environment file in the Servebolt hosting environment.'); ?></p>
            <?php if (!$serveboltCachePurge->configurationOk()) : ?>
                <p style="font-weight: normal;color: red;"><?php _e('We could not seem to load the configuration from the environment file and therefore the cache purge feature is disabled.'); ?></p>
            <?php endif; ?>
        </th>
    </tr>

    <tr>
        <th scope="row"><label for="sb_api_key"><?php _e('API key'); ?></label></th>
        <td>
    ...
    <tr>
        <th scope="row"><label for="sb_environment_id"><?php _e('Environment ID'); ?></label></th>
        <td>
