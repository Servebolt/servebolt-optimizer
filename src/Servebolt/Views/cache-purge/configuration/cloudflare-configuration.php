...
<tbody class="sb-config-field-general sb-config-field-cloudflare <?php if ( ! $settings['cache_purge_switch'] || $settings['cache_purge_driver'] !== 'cloudflare' ) echo 'sb-config-field-hidden'; ?>">
    <tr>
        <th scope="row" colspan="100%" style="padding-bottom: 5px;">
            <h3 style="margin-bottom: 0;"><?php _e('API credentials'); ?></h3>
    ...
    <tr>
        <th scope="row"><?php _e('Authentication type'); ?></th>
        <td>
            <fieldset>
                <legend class="screen-reader-text"><span><?php _e('Authentication type'); ?></span></legend>
                <label><input type="radio" name="<?php echo sb_get_option_name('cf_auth_type'); ?>" value="api_token" <?php checked($settings['cf_auth_type'] == 'api_token'); ?>> <code><?php _e('API token'); ?></code></label><br>
                <label><input type="radio" name="<?php echo sb_get_option_name('cf_auth_type'); ?>" value="api_key" <?php checked($settings['cf_auth_type'] == 'api_key'); ?>> <code><?php _e('API key'); ?></code></label>
            </fieldset>
        </td>
    </tr>
    <tr class="feature_cf_auth_type-api_token"<?php if ( $settings['cf_auth_type'] != 'api_token' ) echo ' style="display: none;"' ?>>
        <th scope="row"><label for="sb_cf_api_token"><?php _e('API token'); ?></label></th>
        <td>
            ...
            <p><small><?php echo sprintf(__('Make sure to add permissions for %s when creating a token.'), $cfApi->apiPermissionsNeeded()); ?></small></p>
        </td>
    </tr>
    <tr class="feature_cf_auth_type-api_key"<?php if ( $settings['cf_auth_type'] != 'api_key' ) echo ' style="display: none;"' ?>>
        <th scope="row"><label for="sb_cf_email"><?php _e('Cloudflare e-mail'); ?></label></th>
        <td>
    ...
    <tr class="feature_cf_auth_type-api_key"<?php if ( $settings['cf_auth_type'] != 'api_key' ) echo ' style="display: none;"' ?>>
        <th scope="row"><label for="sb_cf_api_key"><?php _e('API key'); ?></label></th>
        <td>
            <div class="sb-pwd">
    ...
    <tr>
        <th scope="row" colspan="100%" style="padding-bottom: 5px;">
            <h3 style="margin-bottom: 0;"><?php _e('Cloudflare zone'); ?></h3>
            <p style="font-weight: normal;"><?php _e('A domain in Cloudflare is called a zone. We\'ll need the ID of the zone you\'d like to connect here. You can find the Zone ID in the Cloudflare Overview tab of the domain you\'d like to connect, in the right sidebar under the API section.'); ?></p>
        </th>
    </tr>
    <tr>
        <th scope="row"><label for="zone_id"><?php _e('Zone ID'); ?></label></th>
        <td>
            ...
            <p class="active-zone"<?php if ( ! isset($selectedCfZone) || ! $selectedCfZone ) echo ' style="display: none;"'; ?>><?php _e('Selected zone:'); ?> <span><?php if ( isset($selectedCfZone) && $selectedCfZone ) echo $selectedCfZone->name; ?></span></p>

            <div class="zone-selector-container"<?php if ( ! $haveZones ) echo ' style="display: none;"'; ?>>
                <p style="margin-top: 10px;"><?php _e('Available zones:'); ?></p>
