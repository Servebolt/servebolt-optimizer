...
<tbody class="sb-config-field-general sb-config-field-cloudflare <?php if (!$settings['cache_purge_switch']) echo 'sb-config-field-hidden'; ?>">
    ...
    <tr>
        <th scope="row" colspan="100%" style="padding-bottom: 0;">
            <h3 style="margin-bottom: 0;"><?php _e('Cron setup'); ?></h3>
            <p style="font-weight: normal;"><?php echo sprintf(__('Use this feature to trigger cache bust by cron instead of doing it immediately. The cron task is set to run every 1 minute. We recommend that you set WordPress up to use the UNIX-based cron. Read about how to achieve this %shere%s.'), '<a href="https://servebo.lt/vkr8-" target="_">', '</a>'); ?></p>
        </th>
    </tr>
    <tr>
        <th scope="row"><?php _e('Cache purge via cron'); ?></th>
        <td>
            <fieldset>
                <legend class="screen-reader-text"><span><?php _e('Cache purge via cron'); ?></span></legend>
                <label for="cf_cron_purge">
                    ...
                    <?php endif; ?>
                    <?php _e('Use cron to purge cache?'); ?>
                    <?php if ( sb_cf_cache()->cron_state_is_overridden() ) : ?>
