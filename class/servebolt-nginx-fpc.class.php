...
class Servebolt_Nginx_Fpc {
	...
	/**
	 * @var array Cacheable post types.
	 */
	static $post_types = [];
	...
	static function set_headers( $posts ) {
		...
		$excluded_ids = sb_get_option('fpc_exclude');
		...
		elseif ( ( is_front_page() || is_singular() || is_page() ) && array_key_exists( get_post_type(), self::cacheable_post_types() ) && self::cacheable_post_types()[$post_type] === 'on' ) {
			// Make sure the post type can be cached
			self::$post_types[] = get_post_type();
			self::cache_headers();
		}
	...
	static function cacheable_post_types() {
		...
		$post_types = sb_get_option('fpc_settings'); // Get from admin settings instead
